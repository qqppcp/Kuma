...
texture_system::texture_system_state* texture_system::state_ptr = 0;

b8 texture_system::initialize(u64* memory_requirement, void* state, texture_system_config config)
{
    ...
    *memory_requirement = struct_requirement + array_requirement + hashtable_requirement;
    ...
    state_ptr = static_cast<texture_system_state*>(state);
    state_ptr->config = config;

    // The array block is after the state. Already allocated, so just set the pointer.
    void* array_block = (char*)state + struct_requirement;
    state_ptr->registered_textures = static_cast<texture*>(array_block);
...
void texture_system::shutdown(void* state)
{
    if (state_ptr) {
...
texture* texture_system::acquire_by_name(const char* name, b8 auto_release)
{
...
texture* texture_system::aquire_writeable(const char* name, u32 width, u32 height, u8 channel_count, b8 has_transparency) {
    u32 id = INVALID_ID;
...
void texture_system::release_by_name(const char* name)
{
...
texture* texture_system::wrap_internal(const char* name, u32 width, u32 height, u8 channel_count, b8 has_transparency, b8 is_writeable, b8 register_texture, void* internal_data) {
    u32 id = INVALID_ID;
    texture* t = 0;
    if (register_texture) {
        // NOTE: Wrapped textures are never auto-released because it means that thier
...
b8 texture_system::set_internal(texture* t, void* internal_data) {
    if (t) {
        t->internal_data = internal_data;
        t->generation++;
        return true;
...
b8 texture_system::resize(texture* t, u32 width, u32 height, b8 regenerate_internal_data) {
    if (t) {
...
void texture_system::texture_load_job_success(void* params)
{
    ...
    *texture_params->out_texture = texture_params->temp_texture;
    ...
    KMemory::zero_memory(&old, sizeof(texture));
...
void texture_system::texture_load_job_fail(void* params)
{
    texture_load_params* texture_params = (texture_load_params*)params;
...
b8 texture_system::texture_load_job_start(void* params, void* result_data)
{
    ...
    KMemory::copy_memory(result_data, load_params, sizeof(texture_load_params));
...
void texture_system::destroy_default_textures(texture_system_state* state)
{
    if (state) {
...
void texture_system::destroy_texture(texture* t)
{
    // Clean up backend resources.
    renderer_texture_destroy(t);

    KMemory::zero_memory(t->name, sizeof(char) * TEXTURE_NAME_MAX_LENGTH);
    KMemory::zero_memory(t, sizeof(texture));
    t->id = INVALID_ID;
    t->generation = INVALID_ID;
}
