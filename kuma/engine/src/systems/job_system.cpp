...
void store_result(pfn_job_on_complete callback, u32 param_size, void* params) {
    ...
    if (entry.param_size > 0) {
        ...
        entry.params = KMemory::allocate(param_size, MEMORY_TAG_JOB);
        KMemory::copy_memory(entry.params, params, param_size);
    } else {
        entry.params = 0;
    }
...
u32 job_thread_run(void* params) {
    ...
    while (true) {
        ...
        if (info.entry_point) {
            ...
            if (info.param_data) {
                KMemory::free(info.param_data, info.param_data_size, MEMORY_TAG_JOB);
            }
            if (info.result_data) {
                KMemory::free(info.result_data, info.result_data_size, MEMORY_TAG_JOB);
            }
            ...
            KMemory::zero_memory(&thread->info, sizeof(job_info));
            if (!kmutex_unlock(&thread->info_mutex)) {
                KERROR("Failed to release lock on job thread mutex!");
            }
        }
...
b8 job_system_initialize(u64* job_system_memory_requirement, void* state, u8 job_thread_count, u32 type_masks[]) {
    *job_system_memory_requirement = sizeof(job_system_state);
    ...
    KMemory::zero_memory(state, sizeof(job_system_state));

    state_ptr = (job_system_state*)state;
    ...
    for (u8 i = 0; i < state_ptr->thread_count; ++i) {
        ...
        KMemory::zero_memory(&state_ptr->job_threads[i].info, sizeof(job_info));
    }
...
void job_system_update() {
    ...
    for (u16 i = 0; i < MAX_JOB_RESULTS; ++i) {
        ...
        if (entry.id != INVALID_ID_U16) {
            ...
            if (entry.params) {
                KMemory::free(entry.params, entry.param_size, MEMORY_TAG_JOB);
            }
            ...
            KMemory::zero_memory(&state_ptr->pending_results[i], sizeof(job_result_entry));
            state_ptr->pending_results[i].id = INVALID_ID_U16;
            if (!kmutex_unlock(&state_ptr->result_mutex)) {
                KERROR("Failed to release lock on result mutex!");
            }
...
job_info job_create_priority(pfn_job_start entry_point, pfn_job_on_complete on_success, pfn_job_on_complete on_fail, void* param_data, u32 param_data_size, u32 result_data_size, job_type type, job_priority priority) {
    ...
    if (param_data_size) {
        job.param_data = KMemory::allocate(param_data_size, MEMORY_TAG_JOB);
        KMemory::copy_memory(job.param_data, param_data, param_data_size);
    } else {
        job.param_data = 0;
    }
    ...
    if (result_data_size) {
        job.result_data = KMemory::allocate(result_data_size, MEMORY_TAG_JOB);
    } else {
        job.result_data = 0;
    }
