...
texture* texture_system::texture_system_acquire(const char* name, b8 auto_release)
{
...
void texture_system::texture_system_release(const char* name)
{
...
texture* texture_system::texture_system_get_default_texture()
{
    if (state_ptr) {
        return &state_ptr->default_texture;
    }
...
b8 texture_system::create_default_textures(texture_system_state* state)
{
    ...
    KMemory::set_memory(pixels, 255, sizeof(u8) * pixel_count * channels);
...
b8 texture_system::load_texture(const char* texture_name, texture* t)
{
    ...
    char* format_str = (char*)"D:/Dev/Kuma++/code/kuma/assets/textures/%s.%s";
