...
void geometry_system_config_dispose(geometry_config* config) {
    if (config) {
        if (config->vertices) {
            KMemory::free(config->vertices, config->vertex_size * config->vertex_count, MEMORY_TAG_ARRAY);
        }
        if (config->vertices) {
            KMemory::free(config->indices, config->index_size * config->index_count, MEMORY_TAG_ARRAY);
        }
        KMemory::zero_memory(config, sizeof(geometry_config));
    }
}
...
b8 create_default_geometries(geometry_system_state* state) {
    ...
    vertex_2d verts2d[4];
    KMemory::zero_memory(verts2d, sizeof(vertex_2d) * 4);
    ...
    // Acquire the default material.
    state->default_2d_geometry.material = material_system::get_default_material();
...
geometry_config geometry_system_generate_cube_config(f32 width, f32 height, f32 depth, f32 tile_x, f32 tile_y, const char* name, const char* material_name) {
    ...
    config.vertices = KMemory::allocate(sizeof(vertex_3d) * config.vertex_count, MEMORY_TAG_ARRAY);
    config.index_size = sizeof(u32);
    config.index_count = 6 * 6;  // 6 indices per side, 6 sides
    config.indices = KMemory::allocate(sizeof(u32) * config.index_count, MEMORY_TAG_ARRAY);
    ...
    KMemory::copy_memory(config.vertices, verts, config.vertex_size * config.vertex_count);
    ...
    geometry_generate_tangents(config.vertex_count, (vertex_3d*)config.vertices, config.index_count, (u32*)config.indices);
