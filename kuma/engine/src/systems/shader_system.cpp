...
b8 shader_system_initialize(u64* memory_requirement, void* memory, shader_system_config config) {
    ...
    *memory_requirement = struct_requirement + hashtable_requirement + shader_array_requirement;
    ...
    state_ptr = static_cast<shader_system_state*>(memory);
    u64 addr = (u64)memory;
    state_ptr->lookup_memory = (void*)(addr + struct_requirement);
    state_ptr->shaders = (shader*)((u64)state_ptr->lookup_memory + hashtable_requirement);
...
void shader_system_shutdown(void* state) {
    if (state) {
        ...
        KMemory::zero_memory(st, sizeof(shader_system_state));
    }
...
b8 shader_system_create(const shader_config* config) {
    ...
    KMemory::zero_memory(out_shader, sizeof(shader));
    out_shader->id = id;
    ...
    // Setup arrays
    out_shader->global_texture_maps = (texture_map**)darray_create(texture_map*);
    out_shader->uniforms = static_cast<shader_uniform*>(darray_create(shader_uniform));
    out_shader->attributes = static_cast<shader_attribute*>(darray_create(shader_attribute));
    ...
    out_shader->hashtable_block = KMemory::allocate(element_size * element_count, MEMORY_TAG_UNKNOWN);
...
void shader_destroy(shader* s) {
    ...
    for (u32 i = 0; i < sampler_count; ++i) {
        KMemory::free(s->global_texture_maps[i], sizeof(texture_map), MEMORY_TAG_RENDERER);
    }
    ...
    if (s->name) {
        ...
        KMemory::free(s->name, length + 1, MEMORY_TAG_STRING);
    }
    s->name = 0;
}
...
b8 add_sampler(shader* shader, shader_uniform_config* config) {
    ...
    if (config->scope == SHADER_SCOPE_GLOBAL) {
        ...
        texture_map* map = (texture_map*)KMemory::allocate(sizeof(texture_map), MEMORY_TAG_RENDERER);
        *map = default_map;
        map->texture = texture_system::get_default_texture();
