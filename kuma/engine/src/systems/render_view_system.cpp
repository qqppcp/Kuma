...
b8 render_view_system_initialize(u64* memory_requirement, void* state, render_view_system_config config) {
    ...
    *memory_requirement = struct_requirement + hashtable_requirement + array_requirement;
    ...
    state_ptr = (render_view_system_state*)state;
    ...
    state_ptr->registered_views = (render_view*)((u64)state_ptr->table_block + hashtable_requirement);
...
b8 render_view_system_create(const render_view_config* config) {
    ...
    view->passes = (renderpass**)KMemory::allocate(sizeof(renderpass*) * view->renderpass_count, MEMORY_TAG_ARRAY);
    ...
    if (!view->on_create(view)) {
        ...
        KMemory::free(view->passes, sizeof(renderpass*) * view->renderpass_count, MEMORY_TAG_ARRAY);
        KMemory::zero_memory(&state_ptr->registered_views[id], sizeof(render_view));
        return false;
    }
