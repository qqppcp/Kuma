...
void mesh_load_job_success(void* params) {
    ...
    mesh_params->out_mesh->geometries = (geometry**)KMemory::allocate(sizeof(geometry*) * mesh_params->out_mesh->geometry_count, MEMORY_TAG_ARRAY);
...
void mesh_unload(mesh* m) {
    if (m) {
        ...
        KMemory::free(m->geometries, sizeof(geometry*) * m->geometry_count, MEMORY_TAG_ARRAY);
        KMemory::zero_memory(m, sizeof(mesh));
