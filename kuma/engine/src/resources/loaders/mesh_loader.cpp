...
b8 load_ksm_file(file_handle* ksm_file, geometry_config** out_geometries_darray) {
    ...
    for (u32 i = 0; i < geometry_count; ++i) {
        ...
        g.vertices = KMemory::allocate(g.vertex_size * g.vertex_count, MEMORY_TAG_ARRAY);
        ...
        g.indices = KMemory::allocate(g.index_size * g.index_count, MEMORY_TAG_ARRAY);
        ...
        auto array = *out_geometries_darray;
        darray_push(array, g);
        *out_geometries_darray = array;
    }
...
b8 import_obj_file(file_handle* obj_file, const char* out_ksm_filename, geometry_config** out_geometries_darray) {
    // Positions
    vec3* positions = (vec3*)darray_reserve(vec3, 16384);

    // Normals
    vec3* normals = (vec3*)darray_reserve(vec3, 16384);

    // Normals
    vec2* tex_coords = (vec2*)darray_reserve(vec2, 16384);

    // Groups
    mesh_group_data* groups = (mesh_group_data*)darray_reserve(mesh_group_data, 4);
    ...
    while (true) {
        ...
        switch (first_char) {
            ...
            case 'u': {
                    ...
                    new_group.faces = (mesh_face_data*)darray_reserve(mesh_face_data, 16384);
            ...
            case 'g': {
                ...
                for (u64 i = 0; i < group_count; ++i) {
                    ...
                    auto array = *out_geometries_darray;
                    darray_push(array, new_data);
                    *out_geometries_darray = array;

                    // Increment the number of objects.
                    darray_destroy(groups[i].faces);
                    KMemory::zero_memory(material_names[i], 64);
                }
                current_mat_name_count = 0;
                darray_clear(groups);
                KMemory::zero_memory(name, 512);
    ...
    for (u64 i = 0; i < count; ++i) {
        ...
        geometry_deduplicate_vertices(g->vertex_count, (vertex_3d*)g->vertices, g->index_count, (u32*)g->indices, &new_vert_count, &unique_verts);
        ...
        u32* indices = (u32*)KMemory::allocate(sizeof(u32) * g->index_count, MEMORY_TAG_ARRAY);
        KMemory::copy_memory(indices, g->indices, sizeof(u32) * g->index_count);
        // Destroy the darray
        ...
        geometry_generate_tangents(g->vertex_count, (vertex_3d*)g->vertices, g->index_count, (u32*)g->indices);
    }
...
void process_subobject(vec3* positions, vec3* normals, vec2* tex_coords, mesh_face_data* faces, geometry_config* out_data) {
    ...
    b8 extent_set = false;
    KMemory::zero_memory(&out_data->max_extents, sizeof(vec3));
    KMemory::zero_memory(&out_data->min_extents, sizeof(vec3));
...
b8 import_obj_material_library_file(const char* mtl_file_path) {
    ...
    KMemory::zero_memory(&current_config, sizeof(current_config));
        ...
        switch (first_char) {
            ...
            case 'n': {
                ...
                if (strings_nequali(substr, "newmtl", 6)) {
                    ...
                    current_config.shader_name = (char*)"Shader.Builtin.Material";
...
b8 write_kmt_file(const char* mtl_file_path, material_config* config) {
    ...
    char* format_str = (char*)"%s../materials/%s%s";
    file_handle f;
    char directory[320] = "";
