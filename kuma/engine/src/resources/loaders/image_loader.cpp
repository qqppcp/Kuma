...
#define IMAGE_EXTENSION_COUNT 4
    ...
    u8* raw_data = (u8*)KMemory::allocate(file_size, MEMORY_TAG_TEXTURE);
    if (!raw_data) {
        KERROR("Unable to read file '%s'.", full_file_path);
        filesystem_close(&f);
        return false;
    ...
    KMemory::free(raw_data, file_size, MEMORY_TAG_TEXTURE);
    
    image_resource_data* resource_data = static_cast<image_resource_data*>(KMemory::allocate(sizeof(image_resource_data), MEMORY_TAG_TEXTURE));
