...
b8 text_loader_load(struct resource_loader* self, const char* name, resource* out_resource) {
    ...
    // TODO: Should be using an allocator here.
    char* resource_data = static_cast<char*>(KMemory::allocate(sizeof(char) * file_size, MEMORY_TAG_ARRAY));
    u64 read_size = 0;
...
void text_loader_unload(struct resource_loader* self, resource* resource) {
    ...
    if (resource->data) {
        KMemory::free(resource->data, resource->data_size, MEMORY_TAG_ARRAY);
        resource->data = 0;
