...
b8 material_loader_load(struct resource_loader* self, const char* name, resource* out_resource) {
    ...
    // TODO: Should be using an allocator here.
    material_config* resource_data = static_cast<material_config*>(KMemory::allocate(sizeof(material_config), MEMORY_TAG_MATERIAL_INSTANCE));
    // Set some defaults.
...
void material_loader_unload(struct resource_loader* self, resource* resource) {
    ...
    if (resource->data) {
        KMemory::free(resource->data, resource->data_size, MEMORY_TAG_MATERIAL_INSTANCE);
        resource->data = 0;
