...
b8 shader_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource) {
    ...
    shader_config* resource_data = static_cast<shader_config*>(KMemory::allocate(sizeof(shader_config), MEMORY_TAG_RESOURCE));
    // Set some defaults, create arrays.
    resource_data->attribute_count = 0;
    resource_data->attributes = static_cast<shader_attribute_config*>(darray_create(shader_attribute_config));
    resource_data->uniform_count = 0;
    resource_data->uniforms = static_cast<shader_uniform_config*>(darray_create(shader_uniform_config));
    resource_data->stage_count = 0;
    resource_data->stages = static_cast<shader_stage*>(darray_create(shader_stage));
    resource_data->cull_mode = FACE_CULL_MODE_BACK;
    resource_data->stage_count = 0;
    resource_data->stage_names = static_cast<char**>(darray_create(char*));
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length)) {
        ...
        KMemory::zero_memory(raw_var_name, sizeof(char) * 64);
        ...
        char raw_value[446];
        KMemory::zero_memory(raw_value, sizeof(char) * 446);
        ...
        if (strings_equali(trimmed_var_name, "version")) {
            ...
            // Parse attribute.
            //char** fields = static_cast<char**>(darray_create(char*));
            std::vector<char* , MyAllc<char*>> fields;
        ...
        KMemory::zero_memory(line_buf, sizeof(char) * 512);
        line_number++;
    }
...
void shader_loader_unload(struct resource_loader* self, resource* resource) {
    ...
    for (u32 i = 0; i < count; ++i) {
        ...
        KMemory::free(data->attributes[i].name, sizeof(char) * (len + 1), MEMORY_TAG_STRING);
    }
    ...
    for (u32 i = 0; i < count; ++i) {
        ...
        KMemory::free(data->uniforms[i].name, sizeof(char) * (len + 1), MEMORY_TAG_STRING);
    }
    darray_destroy(data->uniforms);

    KMemory::free(data->renderpass_name, sizeof(char) * (string_length(data->renderpass_name) + 1), MEMORY_TAG_STRING);
