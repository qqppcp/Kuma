...
b8 KMemory::memory_system_initialize(memory_system_configuration config) {
...
void KMemory::memory_system_shutdown()
{
    if (state_ptr) {
...
void* KMemory::allocate(u64 size, memory_tag tag)
{
...
void KMemory::free(void* block, u64 size, memory_tag tag)
{
    if (tag == MEMORY_TAG_UNKNOWN) {
...
void* KMemory::zero_memory(void* block, u64 size)
{
    return platform_zero_memory(block, size);
}

void* KMemory::copy_memory(void* dest, const void* source, u64 size)
{
    return platform_copy_memory(dest, source, size);
}

void* KMemory::set_memory(void* dest, i32 value, u64 size)
{
    return platform_set_memory(dest, value, size);
}

char* KMemory::get_memory_usage_str()
{
...
u64 KMemory::get_memory_alloc_count() {
    if (state_ptr) {
