...
char* string_duplicate(const char* str) {
    ...
    char* copy = (char*)KMemory::allocate(length + 1, MEMORY_TAG_STRING);
    KMemory::copy_memory(copy, str, length);
    copy[length] = 0;
    return copy;
}
...
i32 string_format_v(char* dest, const char* format, void* va_listp) {
    if (dest) {
        ...
        char buffer[32000];
        i32 written = vsnprintf(buffer, 32000, format, static_cast<va_list>(va_listp));
        buffer[written] = 0;
        KMemory::copy_memory(dest, buffer, written + 1);
...
b8 string_to_vec4(char* str, vec4* out_vector) {
    ...
    KMemory::zero_memory(out_vector, sizeof(vec4));
...
b8 string_to_vec3(char* str, vec3* out_vector) {
    ...
    KMemory::zero_memory(out_vector, sizeof(vec3));
...
b8 string_to_vec2(char* str, vec2* out_vector) {
    ...
    KMemory::zero_memory(out_vector, sizeof(vec2));
    ...
    for (u32 i = 0; i < length; ++i) {
        ...
        if (c == delimiter) {
            ...
            if (trimmed_length > 0 || include_empty) {
                char* entry = (char*)KMemory::allocate(sizeof(char) * (trimmed_length + 1), MEMORY_TAG_STRING);
                if (trimmed_length == 0) {
                    entry[0] = 0;
                } else {
            ...
            // Clear the buffer.
            KMemory::zero_memory(buffer, sizeof(char) * 16384);
            current_length = 0;
            continue;
        }
...
void string_cleanup_split_array(char** str_darray) {
    if (str_darray) {
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            KMemory::free(str_darray[i], sizeof(char) * (len + 1), MEMORY_TAG_STRING);
        }
