...
b8 application_create(game* game_inst) {
    ...
    cube_map->texture = texture_system::acquire_cube("skybox", true);
    ...
    if (!resource_system_load("sponza", RESOURCE_TYPE_MESH, 0, &sponza_mesh_resource)) {
        ...
        sponza_mesh->geometries = static_cast<geometry**>(KMemory::allocate(sizeof(mesh*) * sponza_mesh->geometry_count, MEMORY_TAG_ARRAY));
        ...
        sponza_mesh->transform = transform_from_position_rotation_scale({15.0f, 0.0f, 1.0f}, quat_identity(), vec3_create(0.05f, 0.05f, 0.05f));
    ...
    app_state->ui_meshes[0].geometries = (geometry**)KMemory::allocate(sizeof(geometry*), MEMORY_TAG_ARRAY);
...
b8 application_run() {
    ...
    while (app_state->is_running) {
        ...
        if (!app_state->is_suspended) {
            ...
            KMemory::zero_memory(views, sizeof(render_view_packet) * packet.view_count);
            packet.views = views;
