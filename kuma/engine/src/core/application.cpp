...
b8 application_create(game* game_inst) {
    ...
    texture_system::initialize(&app_state->texture_system_memory_requirement, 0, texture_sys_config);
    ...
    if (!texture_system::initialize(&app_state->texture_system_memory_requirement, app_state->texture_system_state, texture_sys_config)) {
    ...
    material_system::initialize(&app_state->material_system_memory_requirement, 0, material_sys_config);
    ...
    if (!material_system::initialize(&app_state->material_system_memory_requirement, app_state->material_system_state, material_sys_config)) {
    ...
    // TODO: temp 
    app_state->meshes = static_cast<mesh*>(darray_create(mesh));
    ...
    KMemory::free(g_config.vertices, sizeof(vertex_3d) * g_config.vertex_count, MEMORY_TAG_ARRAY);
    KMemory::free(g_config.indices, sizeof(u32) * g_config.index_count, MEMORY_TAG_ARRAY);
    ...
    geometry_generate_tangents(g_config_2.vertex_count, (vertex_3d*)g_config_2.vertices, g_config_2.index_count, (u32*)g_config_2.indices);
    cube_mesh_2.geometries[0] = geometry_system_acquire_from_config(g_config, true);
    cube_mesh_2.model = mat4_translation({10.0f, 0.0f, 1.0f});
    darray_push(app_state->meshes, cube_mesh_2);
    // Clean up the allocations for the geometry config.
    KMemory::free(g_config_2.vertices, sizeof(vertex_3d) * g_config_2.vertex_count, MEMORY_TAG_ARRAY);
    KMemory::free(g_config_2.indices, sizeof(u32) * g_config_2.index_count, MEMORY_TAG_ARRAY);
...
b8 application_run() {
    ...
    while (app_state->is_running) {
        ...
        if (!app_state->is_suspended) {
            ...
            if (mesh_count > 0) {
                ...
                packet.geometries = static_cast<geometry_render_data*>(darray_create(geometry_render_data));

                // Perform a small rotation on the first mesh.
                quat rotation = quat_from_axis_angle({0, 1, 0}, 0.5f * delta, false);
                ...
                if (mesh_count > 1) {
                    ...
                    app_state->meshes[1].model = mat4_mul(mat4_translation({10.0f, 0.0f, 1.0f}), app_state->meshes[0].model);
                }
            ...
            test_ui_render.model = mat4_translation({0, 0, 0});
    ...
    material_system::shutdown(app_state->material_system_state);
    
    texture_system::shutdown(app_state->texture_system_state);
