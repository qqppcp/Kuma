...
b8 event_on_debug_event(u16 code, void* sender, void* listener_inst, event_context data) {
    if (code == EVENT_CODE_DEBUG0) {
        ...
        if (g) {
            ...
            material_system::release_by_name(old_name);
        }
        return true;
...
b8 application_create(game* game_inst) {
    ...
    app_state->car_mesh->transform = transform_from_position({15.0f, 0.0f, 1.0f});
    mesh_count++;

    app_state->sponza_mesh = &app_state->meshes[mesh_count];
    app_state->sponza_mesh->transform = transform_from_position_rotation_scale({15.0f, 0.0f, 1.0f}, quat_identity(), {0.05f, 0.05f, 0.05f});
    mesh_count++;
