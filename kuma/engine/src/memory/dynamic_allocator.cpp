...
b8 dynamic_allocator_create(u64 total_size, u64* memory_requirement, void* memory, dynamic_allocator* out_allocator) {
    ...
    *memory_requirement = freelist_requirement + sizeof(dynamic_allocator_state) + total_size;
    ...
    KMemory::zero_memory(state->memory_block, total_size);
    return true;
}
...
b8 dynamic_allocator_destroy(dynamic_allocator* allocator) {
    if (allocator) {
        ...
        KMemory::zero_memory(state->memory_block, state->total_size);
        state->total_size = 0;
        allocator->memory = 0;
        return true;
    }
...
b8  dynamic_allocator_free(dynamic_allocator* allocator, void* block, u64 size) {
    ...
    dynamic_allocator_state* state = (dynamic_allocator_state*)allocator->memory;
    if (block < (char*)state->memory_block || block > (char*)state->memory_block + state->total_size) {
    ...
    u64 offset = ((char*)block - state->memory_block);
