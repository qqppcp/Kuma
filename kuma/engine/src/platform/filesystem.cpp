...
b8 filesystem_read_line(file_handle* handle, char** line_buf) {
    if (handle->handle) {
        ...
        if (fgets(buffer, 32000, (FILE*)handle->handle) != 0) {
            ...
            *line_buf = static_cast<char*>(KMemory::allocate((sizeof(char) * length) + 1, MEMORY_TAG_STRING));
...
b8 filesystem_read_all_bytes(file_handle* handle, u8** out_bytes, u64* out_bytes_read) {
    if (handle->handle) {
        ...
        *out_bytes = static_cast<u8*>(KMemory::allocate(sizeof(u8) * size, MEMORY_TAG_STRING));
