...
b8 render_view_world_on_create(struct render_view* self) {
    if (self) {
        self->internal_data = KMemory::allocate(sizeof(render_view_world_internal_data), MEMORY_TAG_RENDERER);
        render_view_world_internal_data* data = (render_view_world_internal_data*)self->internal_data;
        ...
        data->ambient_colour = {0.25f, 0.25f, 0.25f, 1.0f};
...
void render_view_world_on_destroy(struct render_view* self) {
    if (self && self->internal_data) {
        ...
        KMemory::free(self->internal_data, sizeof(render_view_world_internal_data), MEMORY_TAG_RENDERER);
...
b8 render_view_world_on_build_packet(const struct render_view* self, void* data, struct render_view_packet* out_packet) {
    ...
    out_packet->geometries = (geometry_render_data*)darray_create(geometry_render_data);
    ...
    geometry_distance* geometry_distances = (geometry_distance*)darray_create(geometry_distance);
...
b8 render_view_world_on_render(const struct render_view* self, const struct render_view_packet* packet, u64 frame_number, u64 render_target_index) {
    ...
    for (u32 p = 0; p < self->renderpass_count; ++p) {
        ...
        if (!material_system::apply_global(shader_id, frame_number, &packet->projection_matrix, &packet->view_matrix, &packet->ambient_colour, &packet->view_position, data->render_mode)) {
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            if (!material_system::apply_instance(m, needs_update)) {
            ...
            // Apply the locals
            material_system::apply_local(m, &packet->geometries[i].model);
