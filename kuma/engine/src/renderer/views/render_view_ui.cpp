...
b8 render_view_ui_on_create(struct render_view* self) {
    if (self) {
        self->internal_data = KMemory::allocate(sizeof(render_view_ui_internal_data), MEMORY_TAG_RENDERER);
        render_view_ui_internal_data* data = (render_view_ui_internal_data*)self->internal_data;
...
void render_view_ui_on_destroy(struct render_view* self) {
    if (self && self->internal_data) {
        KMemory::free(self->internal_data, sizeof(render_view_ui_internal_data), MEMORY_TAG_RENDERER);
...
b8 render_view_ui_on_render(const struct render_view* self, const struct render_view_packet* packet, u64 frame_number, u64 render_target_index) {
    ...
    for (u32 p = 0; p < self->renderpass_count; ++p) {
        ...
        // Apply globals
        if (!material_system::apply_global(shader_id, frame_number, &packet->projection_matrix, &packet->view_matrix, 0, 0, 0)) {
