...
b8 render_view_skybox_on_create(struct render_view* self) {
    if (self) {
        self->internal_data = KMemory::allocate(sizeof(render_view_skybox_internal_data), MEMORY_TAG_RENDERER);
        render_view_skybox_internal_data* data = (render_view_skybox_internal_data*)self->internal_data;
...
void render_view_skybox_on_destroy(struct render_view* self) {
    if (self && self->internal_data) {
        KMemory::free(self->internal_data, sizeof(render_view_skybox_internal_data), MEMORY_TAG_RENDERER);
