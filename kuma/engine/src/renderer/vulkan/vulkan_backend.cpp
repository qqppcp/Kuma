...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const renderer_backend_config* config, u8* out_window_render_target_count) {
    ...
    *out_window_render_target_count = context.swapchain.image_count;
    ...
    context.renderpass_table_block = KMemory::allocate(sizeof(u32) * VULKAN_MAX_REGISTERED_RENDERPASSES, MEMORY_TAG_RENDERER);
    ...
    // Create sync objects.
    context.image_available_semaphores = static_cast<VkSemaphore*>(darray_reserve(VkSemaphore, context.swapchain.max_frames_in_flight));
    context.queue_complete_semaphores = static_cast<VkSemaphore*>(darray_reserve(VkSemaphore, context.swapchain.max_frames_in_flight));
...
b8 vulkan_renderer_renderpass_begin(renderpass* pass, render_target* target) {
    ...
    begin_info.framebuffer = (VkFramebuffer)target->internal_framebuffer;
    ...
    if (do_clear_colour) {
        KMemory::copy_memory(clear_values[begin_info.clearValueCount].color.float32, pass->clear_colour.elements, sizeof(f32) * 4);
...
void vulkan_renderer_texture_create(const u8* pixels, texture* t) {
    ...
    t->internal_data = (vulkan_image*)KMemory::allocate(sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
...
void vulkan_renderer_texture_destroy(struct texture* texture) {
    ...
    if (image) {
        ...
        KMemory::zero_memory(image, sizeof(vulkan_image));

        KMemory::free(texture->internal_data, sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
    }
    KMemory::zero_memory(texture, sizeof(struct texture));
}
...
void vulkan_renderer_texture_write_data(texture* t, u32 offset, u32 size, const u8* pixels) {
    ...
    vulkan_buffer staging;
    vulkan_buffer_create(&context, image_size, (VkBufferUsageFlagBits)usage, memory_prop_flags, true, false, &staging);
...
void vulkan_renderer_destroy_geometry(geometry* geometry) {
    if (geometry && geometry->internal_id != INVALID_ID) {
        ...
        // Clean up data.
        KMemory::zero_memory(internal_data, sizeof(vulkan_geometry_data));
...
b8 vulkan_renderer_shader_create(shader* shader, const shader_config* config, renderpass* pass, u8 stage_count, const char** stage_filenames, shader_stage* stages) {
    shader->internal_data = KMemory::allocate(sizeof(vulkan_shader), MEMORY_TAG_RENDERER);
    ...
    out_shader->renderpass = (vulkan_renderpass*)pass->internal_data;

    // Build out the configuration.
    out_shader->config.max_descriptor_set_count = max_descriptor_allocate_count;

    // Shader stages. Parse out the flags.
    KMemory::zero_memory(out_shader->config.stages, sizeof(vulkan_shader_stage_config) * VULKAN_SHADER_MAX_STAGES);
    ...
    KMemory::zero_memory(out_shader->config.descriptor_sets, sizeof(vulkan_descriptor_set_config) * 2);
    ...
    // Attributes array.
    KMemory::zero_memory(out_shader->config.attributes, sizeof(VkVertexInputAttributeDescription) * VULKAN_SHADER_MAX_ATTRIBUTES);
...
void vulkan_renderer_shader_destroy(shader* s) {
    if (s && s->internal_data) {
        vulkan_shader* shader = static_cast<vulkan_shader*>(s->internal_data);
        if (!shader) {
        ...
        KMemory::zero_memory(&shader->config, sizeof(vulkan_shader_config));

        // Free the internal data memory.
        KMemory::free(s->internal_data, sizeof(vulkan_shader), MEMORY_TAG_RENDERER);
        s->internal_data = 0;
    }
}
...
b8 vulkan_renderer_shader_initialize(shader* shader) {
    ...
    KMemory::zero_memory(s->stages, sizeof(vulkan_shader_stage) * VULKAN_SHADER_MAX_STAGES);
    ...
    KMemory::zero_memory(s->descriptor_set_layouts, s->config.descriptor_set_count);
    ...
    KMemory::zero_memory(stage_create_infos, sizeof(VkPipelineShaderStageCreateInfo) * VULKAN_SHADER_MAX_STAGES);
    for (u32 i = 0; i < s->config.stage_count; ++i) {
...
b8 vulkan_renderer_shader_use(shader* shader) {
    vulkan_shader* s = static_cast<vulkan_shader*>(shader->internal_data);
...
b8 vulkan_renderer_shader_bind_instance(shader* s, u32 instance_id) {
    ...
    vulkan_shader* internal = static_cast<vulkan_shader*>(s->internal_data);
...
b8 vulkan_renderer_shader_apply_instance(shader* s, b8 needs_update) {
    vulkan_shader* internal = (vulkan_shader*)s->internal_data;
    ...
    KMemory::zero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * 2);
            ...
            for (u32 i = 0; i < total_sampler_count; ++i) {
                ...
                if (t->generation == INVALID_ID) {
                    ...
                    case TEXTURE_USE_MAP_DIFFUSE:
                        t = texture_system::get_default_diffuse_texture();
                        break;
                    case TEXTURE_USE_MAP_SPECULAR:
                        t = texture_system::get_default_specular_texture();
                        break;
                    case TEXTURE_USE_MAP_NORMAL:
                        t = texture_system::get_default_normal_texture();
                        break;
                    default:
                        KWARN("Undefined texture use %d", map->use);
                        t = texture_system::get_default_texture();
                        break;
                    }
                }
...
void vulkan_renderer_texture_map_release_resources(texture_map* map) {
    if (map) {
        vkDestroySampler(context.device.logical_device, (VkSampler)map->internal_data, context.allocator);
        map->internal_data = 0;
    }
}
...
b8 vulkan_renderer_shader_acquire_instance_resources(shader* s, texture_map** maps, u32* out_instance_id) {
    ...
    *out_instance_id = INVALID_ID;
    ...
    instance_state->instance_texture_maps = static_cast<texture_map**>(KMemory::allocate(sizeof(texture_map*) * s->instance_texture_count, MEMORY_TAG_ARRAY));
    texture* default_texture = texture_system::get_default_texture();
    KMemory::copy_memory(instance_state->instance_texture_maps, maps, sizeof(texture_map*) * s->instance_texture_count);
    ...
    KMemory::zero_memory(set_state->descriptor_states, sizeof(vulkan_descriptor_state) * VULKAN_SHADER_MAX_BINDINGS);
...
b8 vulkan_renderer_shader_release_instance_resources(shader* s, u32 instance_id) {
    ...
    KMemory::zero_memory(instance_state->descriptor_set_state.descriptor_states, sizeof(vulkan_descriptor_state) * VULKAN_SHADER_MAX_BINDINGS);

    if (instance_state->instance_texture_maps) {
        KMemory::free(instance_state->instance_texture_maps, sizeof(texture_map*) * s->instance_texture_count, MEMORY_TAG_ARRAY);
...
b8 vulkan_renderer_set_uniform(shader* s, shader_uniform* uniform, const void* value) {
    ...
    if (uniform->type == SHADER_UNIFORM_TYPE_SAMPLER) {
        ...
        if (uniform->scope == SHADER_SCOPE_LOCAL) {
            ...
            KMemory::copy_memory((void*)addr, value, uniform->size);
            if (addr) {
            }
        }
    }
...
b8 create_module(vulkan_shader* shader, vulkan_shader_stage_config config, vulkan_shader_stage* shader_stage) {
    ...
    KMemory::zero_memory(&shader_stage->create_info, sizeof(VkShaderModuleCreateInfo));
...
void vulkan_renderpass_create(renderpass* out_renderpass, f32 depth, u32 stencil, b8 has_prev_pass, b8 has_next_pass) {
    out_renderpass->internal_data = KMemory::allocate(sizeof(vulkan_renderpass), MEMORY_TAG_RENDERER);
    ...
    if (do_clear_depth) {
        ...
        KMemory::zero_memory(&attachment_descriptions[attachment_description_count], sizeof(VkAttachmentDescription));
...
void vulkan_renderpass_destroy(renderpass* pass) {
    if (pass && pass->internal_data) {
        ...
        KMemory::free(internal_data, sizeof(vulkan_renderpass), MEMORY_TAG_RENDERER);
...
void vulkan_renderer_render_target_create(u8 attachment_count, texture** attachments, renderpass* pass, u32 width, u32 height, render_target* out_target) {
    ...
    if (!out_target->attachments) {
        out_target->attachments = static_cast<texture**>(KMemory::allocate(sizeof(texture*) * attachment_count, MEMORY_TAG_ARRAY));
    }
    KMemory::copy_memory(out_target->attachments, attachments, sizeof(texture*) * attachment_count);
...
void vulkan_renderer_render_target_destroy(render_target* target, b8 free_internal_memory) {
    if (target && target->internal_framebuffer) {
        ...
        if (free_internal_memory) {
            KMemory::free(target->attachments, sizeof(texture*) * target->attachment_count, MEMORY_TAG_ARRAY);
