...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const char* application_name) {
    ...
    context.swapchain.framebuffers = static_cast<vulkan_framebuffer*>(darray_reserve(vulkan_framebuffer, context.swapchain.image_count));
    ...
    context.in_flight_fences = static_cast<vulkan_fence*>(darray_reserve(vulkan_fence, context.swapchain.max_frames_in_flight));
    ...
    // by this list.
    context.images_in_flight = static_cast<vulkan_fence**>(darray_reserve(vulkan_fence, context.swapchain.image_count));
