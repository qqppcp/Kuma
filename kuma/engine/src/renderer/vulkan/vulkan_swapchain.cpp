...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    if (!swapchain->images) {
        swapchain->images = (VkImage*)KMemory::allocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views) {
        swapchain->views = (VkImageView*)KMemory::allocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
