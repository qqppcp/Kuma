...
void vulkan_swapchain_destroy(
    ...
    for (u32 i = 0; i < swapchain->image_count; ++i) {
        KMemory::free(swapchain->render_textures[i]->internal_data, sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
    }
}
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    if (!swapchain->render_textures) {
        swapchain->render_textures = (texture**)KMemory::allocate(sizeof(texture*) * swapchain->image_count, MEMORY_TAG_RENDERER);
        // If creating the array, then the internal texture objects aren't created yet either.
        for (u32 i = 0; i < swapchain->image_count; ++i) {
            void* internal_data = KMemory::allocate(sizeof(vulkan_image), MEMORY_TAG_TEXTURE);

            char tex_name[38] = "__internal_vulkan_swapchain_image_0__";
            tex_name[34] = '0' + (char)i;

            swapchain->render_textures[i] = texture_system::wrap_internal(
                tex_name,
        ...
        for (u32 i = 0; i < swapchain->image_count; ++i) {
            ...
            texture_system::resize(swapchain->render_textures[i], swapchain_extent.width, swapchain_extent.height, false);
        }
    }
    ...
    vulkan_image* image = static_cast<vulkan_image*>(KMemory::allocate(sizeof(texture), MEMORY_TAG_TEXTURE));
    vulkan_image_create(
        context,
        TEXTURE_TYPE_2D,
        swapchain_extent.width,
    ...
    context->swapchain.depth_texture = texture_system::wrap_internal(
...
void destroy(vulkan_context* context, vulkan_swapchain* swapchain) {
    ...
    KMemory::free(swapchain->depth_texture->internal_data, sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
