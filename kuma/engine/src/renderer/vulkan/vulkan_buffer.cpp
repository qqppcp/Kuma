...
void cleanup_freelist(vulkan_buffer* buffer) {
    ...
    KMemory::free(buffer->freelist_block, buffer->freelist_memory_requirement, MEMORY_TAG_RENDERER);
...
b8 vulkan_buffer_create(
    ...
    KMemory::zero_memory(out_buffer, sizeof(vulkan_buffer));
    ...
    out_buffer->freelist_block = KMemory::allocate(out_buffer->freelist_memory_requirement, MEMORY_TAG_RENDERER);
...
b8 vulkan_buffer_resize(
    ...
    void* new_block = KMemory::allocate(new_memory_requirement, MEMORY_TAG_RENDERER);
    void* old_block = 0;
    if (!freelist_resize(&buffer->buffer_freelist, &new_memory_requirement, new_block, new_size, &old_block)) {
        ...
        KMemory::free(new_block, new_memory_requirement, MEMORY_TAG_RENDERER);
        return false;
    }
    // Clean up the old memory, then assign the new properties over.
    KMemory::free(old_block, buffer->freelist_memory_requirement, MEMORY_TAG_RENDERER);
...
void vulkan_buffer_load_data(vulkan_context* context, vulkan_buffer* buffer, u64 offset, u64 size, u32 flags, const void* data) {
    ...
    KMemory::copy_memory(data_ptr, data, size);
    vkUnmapMemory(context->device.logical_device, buffer->memory);
}
