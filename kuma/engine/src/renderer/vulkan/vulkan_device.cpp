...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    const b8 present_shares_graphics_queue = static_cast<b8>(context->device.graphics_queue_index == context->device.present_queue_index);
    const b8 transfer_shares_graphics_queue = static_cast<b8>(context->device.graphics_queue_index == context->device.transfer_queue_index);
    u32 index_count = 1;
    ...
    std::vector<u32, MyAllc<u32>> indices(index_count);
    u8 index = 0;
    ...
    for (u32 i = 0; i < index_count; ++i) {
        ...
        //if (indices[i] == static_cast<u32>(context->device.graphics_queue_index)) {
...
void vulkan_device_query_swapchain_support(
    ...
    if (out_support_info->format_count != 0) {
        if (!out_support_info->formats) {
            out_support_info->formats = static_cast<VkSurfaceFormatKHR*>(KMemory::allocate(sizeof(VkSurfaceFormatKHR) * out_support_info->format_count, MEMORY_TAG_RENDERER));
        }
        VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(
            physical_device,
            surface,
    ...
    if (out_support_info->present_mode_count != 0) {
        if (!out_support_info->present_modes) {
            out_support_info->present_modes = static_cast<VkPresentModeKHR*>(KMemory::allocate(sizeof(VkPresentModeKHR) * out_support_info->present_mode_count, MEMORY_TAG_RENDERER));
        }
        VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(
            physical_device,
            surface,
...
b8 physical_device_meets_requirements(
    ...
    std::vector<VkQueueFamilyProperties, MyAllc<VkQueueFamilyProperties>> queue_families(queue_family_count);
    ...
    if (
        ...
        if (out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1) {
            if (out_swapchain_support->formats) {
                KMemory::free(out_swapchain_support->formats, sizeof(VkSurfaceFormatKHR) * out_swapchain_support->format_count, MEMORY_TAG_RENDERER);
            }
            if (out_swapchain_support->present_modes) {
                KMemory::free(out_swapchain_support->present_modes, sizeof(VkPresentModeKHR) * out_swapchain_support->present_mode_count, MEMORY_TAG_RENDERER);
            }
        ...
        if (requirements->device_extension_names) {
            ...
            if (available_extension_count != 0) {
                available_extensions = static_cast<VkExtensionProperties*>(KMemory::allocate(sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER));
                VK_CHECK(vkEnumerateDeviceExtensionProperties(
                    device,
                    0,
                    &available_extension_count,
