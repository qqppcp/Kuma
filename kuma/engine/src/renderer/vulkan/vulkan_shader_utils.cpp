...
b8 create_shader_module(
    ...
    // Build file name.
    char file_name[512];
    string_format(file_name, "D:/Dev/Kuma++/code/kuma/assets/shaders/%s.%s.spv", name, type_str);

    KMemory::zero_memory(&shader_stages[stage_index].create_info, sizeof(VkShaderModuleCreateInfo));
    ...
    if (file_buffer) {
        KMemory::free(file_buffer, sizeof(u8) * size, MEMORY_TAG_STRING);
        file_buffer = 0;
    }
