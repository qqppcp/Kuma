...
b8 renderer_system_initialize(u64* memory_requirement, void* state, const char* application_name) {
    *memory_requirement = sizeof(renderer_system_state);
    ...
    state_ptr->ambient_colour = {0.25f, 0.25f, 0.25f, 1.0f};
...
b8 renderer_draw_frame(render_packet* packet) {
    ...
    if (state_ptr->backend.begin_frame(&state_ptr->backend, packet->delta_time)) {
        ...
        // Apply globals
        if(!material_system::apply_global(state_ptr->material_shader_id, &state_ptr->projection, &state_ptr->view, &state_ptr->ambient_colour, &state_ptr->view_position, state_ptr->render_mode)) {
        ...
        // Apply globals
        if(!material_system::apply_global(state_ptr->ui_shader_id, &state_ptr->ui_projection, &state_ptr->ui_view, 0, 0, 0)) {
        ...
        for (u32 i = 0; i < count; ++i) {
            ...
            // Apply the locals
            material_system::apply_local(m, &packet->ui_geometries[i].model);
