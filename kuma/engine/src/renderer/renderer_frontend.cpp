...
b8 event_on_debug_event(u16 code, void* sender, void* listener_inst, event_context data) {
    ...
    state_ptr->test_material->diffuse_map.texture = texture_system::acquire_by_name(names[choice], true);
    if (!state_ptr->test_material->diffuse_map.texture) {
        KWARN("event_on_debug_event no texture! using default");
        state_ptr->test_material->diffuse_map.texture = texture_system::get_default_texture();
    }
...
b8 renderer_draw_frame(render_packet* packet) {
    ...
    if (renderer_begin_frame(packet->delta_time)) {
        ...
        if (!state_ptr->test_material) {
            // Automatic config
            state_ptr->test_material = material_system::acquire_by_name("test_material");
            if (!state_ptr->test_material) {
                ...
                state_ptr->test_material = material_system::acquire_from_config(config);
            }
        }
