...
b8 renderer_draw_frame(render_packet* packet) {
    ...
    if (state_ptr->backend.begin_frame(&state_ptr->backend, packet->delta_time)) {
        ...
        // Apply globals
        if(!material_system::apply_global(state_ptr->material_shader_id, &state_ptr->projection, &state_ptr->view)) {
        ...
        // Apply globals
        if(!material_system::apply_global(state_ptr->ui_shader_id, &state_ptr->ui_projection, &state_ptr->ui_view)) {
