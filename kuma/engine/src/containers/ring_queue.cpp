...
b8 ring_queue_create(u32 stride, u32 capacity, void* memory, ring_queue* out_queue) {
    ...
    if (memory) {
        ...
        out_queue->block = KMemory::allocate(capacity * stride, MEMORY_TAG_RING_QUEUE);
    }
...
void ring_queue_destroy(ring_queue* queue) {
    if (queue) {
        if (queue->owns_memory) {
            KMemory::free(queue->block, queue->capacity * queue->stride, MEMORY_TAG_RING_QUEUE);
        }
        KMemory::zero_memory(queue, sizeof(ring_queue));
    }
}
...
b8 ring_queue_enqueue(ring_queue* queue, void* value) {
    if (queue && value) {
        ...
        KMemory::copy_memory((char*)queue->block + (queue->tail * queue->stride), value, queue->stride);
        queue->length++;
        return true;
    }
...
b8 ring_queue_dequeue(ring_queue* queue, void* out_value) {
    if (queue && out_value) {
        ...
        KMemory::copy_memory(out_value, (char*)queue->block + (queue->head * queue->stride), queue->stride);
