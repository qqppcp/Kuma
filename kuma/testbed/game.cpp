...
b8 game_initialize(game* game_inst) {
    ...
    camera_position_set(state->world_camera, {10.5f, 5.0f, 9.5f});
...
b8 game_update(game* game_inst, f32 delta_time) {
    ...
    alloc_count = KMemory::get_memory_alloc_count();
    if (input_is_key_up((keys)'M') && input_was_key_down((keys)'M')) {
        KDEBUG("Allocations: %llu (%llu this frame)", alloc_count, alloc_count - prev_alloc_count);
    }
    
    // TODO: temp
    if (input_is_key_up((keys)'T') && input_was_key_down((keys)'T')) {
    ...
    if (input_is_key_down((keys)'X')) {
        camera_move_down(state->world_camera, temp_move_speed * delta_time);
    }

    // TODO: temp
    if (input_is_key_up((keys)'P') && input_was_key_down((keys)'P')) {
        KDEBUG(
            "Pos:[%.2f, %.2f, %.2f",
    ...
    if (input_is_key_up((keys)'1') && input_was_key_down((keys)'1')) {
        event_context data = {};
        data.data.i32[0] = RENDERER_VIEW_MODE_LIGHTING;
        event_fire(EVENT_CODE_SET_RENDER_MODE, game_inst, data);
    }

    if (input_is_key_up((keys)'2') && input_was_key_down((keys)'2')) {
        event_context data = {};
        data.data.i32[0] = RENDERER_VIEW_MODE_NORMALS;
        event_fire(EVENT_CODE_SET_RENDER_MODE, game_inst, data);
    }

    if (input_is_key_up((keys)'0') && input_was_key_down((keys)'0')) {
        event_context data = {};
